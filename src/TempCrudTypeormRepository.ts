...
import { CrudRepository } from 'nestjs-rest-crud/src/interfaces/CrudRepository';

export class TempCrudTypeormRepository<T> extends Repository<T>
  ...
  public async createMany(
    ...
    return this.save<any>(bulk, { chunk: 50 });
  }
  ...
  public async updateOne(req: CrudRequest, dto: T): Promise<T> {
    ...
    return this.save<any>({ ...found, ...dto });
  }
  ...
  public async deleteOne(req: CrudRequest): Promise<void | T> {
    const found = await this.getOneOrFail(req);
    const deleted = await this.remove(found);
  ...
  private onInitMapEntityColumns() {
    this.entityColumns = this.metadata.columns.map(prop => {
      this.entityColumnsHash[prop.propertyName] = true;
      return prop.propertyName;
    });
    this.entityPrimaryColumns = this.metadata.columns
      .filter(prop => prop.isPrimary)
      .map(prop => prop.propertyName);
  }

  private onInitMapRelations() {
    this.entityRelationsHash = this.metadata.relations.reduce(
      (hash, curr) => ({
        ...hash,
        [curr.propertyName]: {
  ...
  private getRelationMetadata(field: string) {
    try {
      ...
      let relations = this.metadata.relations;
